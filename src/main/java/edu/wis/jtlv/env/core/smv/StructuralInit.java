...
public class StructuralInit {
	...
	public SMVAbstractElementInfo add_module(Boolean isAgent, String a_name,
		...
		m = new SMVModuleInfo(isAgent, a_name, new SMVParsingInfo(input), args, args_visible);
...
/*	public SMVAbstractElementInfo add_module(Vector<String> name_plus_args,
		...
		return this.add_module(false, module_name, module_arg_list, input);
	}
*/
	...
	public SMVAbstractElementInfo add_module(Vector<String> name_plus_args,
		...
		for (Iterator<String> iter_names = name_plus_args.iterator(); iter_names
				.hasNext();) {
			String a_name = iter_names.next();
			String a_visible = iter_names.next();
			if (is_first) {
				module_name = a_name;
				module_isAgent = a_visible;
			} else {
				// check if a_name is a duplicate argument name
				for (int i = 0; i < module_arg_index; i++) {
					if (module_arg_list[i].equals(a_name)) {
						throw new SMVParseException("The argument name " + a_name + " already exists.", input);
					}
				}
				module_arg_list[module_arg_index] = a_name;

				if (a_visible.equals("0"))
					module_arg_visible_list[module_arg_index] = false;
				else if (a_visible.equals("1"))
					module_arg_visible_list[module_arg_index] = true;
				else {
					throw new SMVParseException("Couldn't determine the visibility of the argument " + a_name + ".", input);
				}
				module_arg_index++;
			}
			is_first = false;
		...
		if (module_isAgent.equals("0")) {
			isAgent = false;
		}else if (module_isAgent.equals("1")) {
			isAgent = true;
		}else{
			throw new SMVParseException("Couldn't determine the module is an agent or not.", input);
		}
		return this.add_module(isAgent, module_name, module_arg_list, module_arg_visible_list, input);
	}
