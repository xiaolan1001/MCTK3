...
import edu.wis.jtlv.env.core.smv.*;
...
public final class Env {
     ...
     */
    private static HashMap<String, Module> all_modules; // including normal modules and agents

    //LXY: for MAS
    private static HashMap<String, SMVAgentInfo> all_agent_modules;

    public static HashMap<String, SMVAgentInfo> getAll_agent_modules() {
     ...
     * </p>
     *
     * @param new_thread The thread to assign a manager to.
     ...
     * </p>
     *
     * @param idx The given BDD manager index.
     * @param b   The BDD to copy.
     ...
     * </p>
     *
     * @param managerIdxs all manager indices
     * @param toCopy      the BDD to copy
     ...
     * </p>
     *
     * @param full_inst_name the full path string to the instance.
     ...
     * </p>
     *
     * @param full_inst_name the string to associate this module with.
     * @param to_add         the module to add.
     * @see edu.wis.jtlv.env.Env#getAllModules()
     * @see edu.wis.jtlv.env.Env#getModule(String)
     * @see edu.wis.jtlv.env.module.Module
     * @see edu.wis.jtlv.env.module.FDSModule
     ...
     * </p>
     *
     * @param filename The file to load with file extension *.fds, or *.smv .
     * @param ordfile  The file with the order of variables. all variable (in their
...
//        String regEx = "((CTLSPEC|LTLSPEC|SPEC|RTCTL\\*SPEC)[^;]*;( *--.*(\r\n)?)?)|((CTLSPEC|LTLSPEC|SPEC|RTCTL\\*SPEC|RTCDL\\*SPEC)[^;]*;( *(\r\n)?)?)";
    ...
    public static Module[] debugParseModule(String filename) throws IOException {
        try {
            if (filename.toLowerCase().endsWith("fds")) {
                return loadFDSModule(new ANTLRFileStream(filename), true, false,
                        null);
            } else if (filename.toLowerCase().endsWith("smv")) {
                return loadSMVModule(new ANTLRFileStream(filename), true, false,
                        null);
            }
        } catch (RecognitionException e) {
            Env.doError(e, e.getMessage());
    ...
    private static SMVModule[] loadSMVModule(CharStream input,
            ...
            throws RecognitionException, ModuleException {
     ...
     * @throws ModuleVariableException If the given name for the new field to create is illegal (for
    ...
    public static BDDVarSet globalUnprimeVarsMinus(BDDVarSet minus) {
        return bdd_namer[Env.getCurrentThreadManagerIdx()]
                .globalUnprimeVarsMinus(minus);
    }
     ...
     * </p>
     *
     * @param e   The exception to invoke the listeners with.
     * @param msg A text message describing the failure.
     * @see edu.wis.jtlv.env.Env#all_error_listeners
     * @see edu.wis.jtlv.env.Env#registerErrorListener(ErrorListener)
     * @see edu.wis.jtlv.env.Env#removeErrorListener(ErrorListener)
     * @see edu.wis.jtlv.env.Env#clearErrorListener()
    ...
    private static class StandardErrorOutput implements ErrorListener {
        public void doError(Exception e, String msg) {
            ...
            //insertDocument(verifyPanel.ctext, "\n "+pre + msg, Color.RED, 1);
        }
    }
    ...
    private static final class JTLVBDDManager {
        ...
        private static final String getProperty(String key, String def) {
            ...
            } catch (AccessControlException ace) { // LXY: the original argument "_" is changed to ace
                return def;
            }
        }
     ...
     * @version {@value edu.wis.jtlv.env.Env#version}
     */
    public static final class JTLVBDDManagerPairing {
        // //////////////////////////////////
       ...
       // private Vector<ModuleBDDField> all_couples; // holds the BDD pairs (unprime and prime versions) for STATE_VARs
         ...
         *
         * @param preface     module name.
         * @param varName     variable name, can also be in prime notation (e.g. x').
         * @param values_size
        ...
        private ModuleBDDField new_var_unprime_only(int thread_idx, String preface,
            ...
            all_couples.add(new_pair);
            System.out.println("preface + \".\" + name"+preface + "." + name);
        ...
        private ModuleBDDField getVarForDomain(BDDDomain dom) {
            for (ModuleBDDField coup : all_couples) {
        ...
        private ModuleBDDField search_couple(String preface, String name) {
            ModuleBDDField[] iter = new ModuleBDDField[all_couples.size()];
            all_couples.toArray(iter);
            for (int i = 0; i < iter.length; i++) {
                if ((iter[i].getPath().equals(preface))
                        && (iter[i].getName().equals(name))) {//&&!preface.startsWith("Tester")
                    return iter[i];
                }
            }
            return null;
    ...
    public static class JTLVBDDToString extends BDDToString implements
        ...
        //TODO bug-solved at 20170425
        //incorrectly generated domain value of integer
