...
class CacheSpecTesterInfo {
    ...
    CacheSpecTesterInfo(    Spec spec,
                            ...
                            BDD feasibleStates  // only store for spec=Ef or Af
    ){
        this.spec=spec;
...
public class RTCDLs_ModelCheckAlg extends ModelCheckAlgI {
    ...
    private RTCDLsTester testerInfo =null;
...
//        BDD specBdd = DTnc1_feas.and(D_feas_negC1.not());
    ...
    public static boolean isTestOnly(Spec sereSpec) throws SpecException {
        if(sereSpec instanceof SpecAgentIdentifier || sereSpec instanceof SpecRange)
            throw new SpecException("Syntax Error: "+ edu.wis.jtlv.lib.mc.RTCDLs_old.RTCDLs_ModelCheckAlg.simplifySpecString(sereSpec,false)+" is not SERE sub-formula.");
    ...
    public BDD satLDLsereSAT(Spec spec) throws SpecException, SMVParseException, ModelCheckException, ModuleException, ModelCheckAlgException {
        ...
        if(ch[0].isPropSpec()){
            ...
            lc = sat(ch[0]); testerInfo.putSpecInfo(ch[0],lc,null,null); // tester.cachePutSpec(ch[0],lc);
            rc = sat(ch[1]); testerInfo.putSpecInfo(ch[1],rc,null,null); // tester.cachePutSpec(ch[1],rc);
            ...
            T.conjunctTrans(xBdd.imp(lc.and(p_rc)));

            testerInfo.putSpecInfo(spec,xBdd,
                    testerInfo.unionTesters(ch[0].toString(),ch[1].toString(),T),null); //           tester.cachePutSpec(spec,xBdd);
            ...
            if(ch0op == Operator.LDL_TEST){
                ...
                if(!ch0ch[0].isLDLSpec(sb))
                    throw new SpecException("Syntax Error: " + simplifySpecString(sb.toString(),false));
                // now ch0ch[0] is LDL sub-formula, and ch[0]=ch0ch[0]?
                BDD b0 = sat(ch0ch[0]).and(sat(ch[1]));
                testerInfo.putSpecInfo(spec,b0,
                        testerInfo.unionTesters(ch0ch[0].toString(),ch[1].toString(),null),null); //tester.cachePutSpec(spec,b0);
                return b0;
                ...
                testerInfo.putSpecInfo(spec0,b0,
                        testerInfo.unionTesters(ch0ch[0].toString(),ch[1].toString(),null),null); //tester.cachePutSpec(spec0,b0);

                Spec spec1=new SpecExp(Operator.LDL_SERE_SAT,ch0ch[1],ch[1]);
                BDD b1=sat(spec1); // b1=sat(<ch0ch[1]>ch[1])
                testerInfo.putSpecInfo(spec1,b1,
                        testerInfo.unionTesters(ch0ch[1].toString(),ch[1].toString(),null),null); //tester.cachePutSpec(spec1,b1);

                BDD b2=b0.or(b1);
                testerInfo.putSpecInfo(spec,b2,
                        testerInfo.unionTesters(spec0.toString(),spec1.toString(),null),null); //tester.cachePutSpec(spec,b2);
                return b2;
                ...
                testerInfo.putSpecInfo(spec,b0,
                        testerInfo.unionTesters(ch0ch[0].toString(),spec1.toString(),null),null); //tester.cachePutSpec(spec,b0);
                return b0;
            }else if(ch0op == Operator.LDL_REPEAT){ // spec = <r*>ch[1] where r=ch0ch[0]
                Spec r=ch0ch[0];
                if(isTestOnly(r)) {
                    Spec spec1=new SpecExp(Operator.LDL_SERE_SAT,r,ch[1]); // spec1=<r>ch[1]
                    return sat(spec1);
                }
                // now r is NOT test-only

                SMVModule T = testerInfo.buildEmptyAtomTester(spec.toString());
                x = T.addVar("X"); // boolean variable
                xBdd = x.getDomain().ithVar(1);
                Spec specX = new SpecBDD(xBdd);
                Spec rho=new SpecExp(Operator.LDL_SERE_SAT,r,specX); // rho = <r>x

                SpecExp rse=(SpecExp) r;
                Operator rop=rse.getOperator();
                Spec[] rch=rse.getChildren();

                if(r.isPropSpec()){

                }else if(rop==Operator.LDL_OR){ // r=r1+r2

                }else if(rop==Operator.LDL_CONC){ // r=r1;r2

                }else if(rop==Operator.LDL_REPEAT){ // r=r1*

                }else{ // rop==Operator.LDL_TEST is impossile because r is test-only
    ...
    public BDD satLDLsereIMP(Spec spec) throws SpecException, SMVParseException, ModelCheckException, ModuleException, ModelCheckAlgException {
        ...
        if(ch[0].isPropSpec()){
            ...
            if(ch0op == Operator.LDL_TEST){
                ...
                StringBuilder sb = new StringBuilder("");
                if(!ch0ch[0].isLDLSpec(sb))
                    throw new SpecException("Syntax Error: " + edu.wis.jtlv.lib.mc.RTCDLs_old.RTCDLs_ModelCheckAlg.simplifySpecString(sb.toString(),false));
                // now ch0ch[0] is LDL sub-formula, and ch[0]=ch0ch[0]?
                ...
                if(edu.wis.jtlv.lib.mc.RTCDLs_old.RTCDLs_ModelCheckAlg.isTestOnly(ch0ch[0])){
...
*/
    ...
    public AlgResultString modelCheckingOneSpec(Spec aProperty) throws SpecException, ModelCheckException, ModuleException, ModelCheckAlgException, SMVParseException {
        ...
        if (D_feas_init_unSat.isZero()) {
            ...
            graph = new GraphExplainRTCDLs(aProperty, negProp, testerInfo, this);
            graph.addAttribute("ui.title", graph.getId());
            // design with the composed tester...
            // create the initial node
