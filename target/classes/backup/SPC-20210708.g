...
ldl_spec					returns[InternalSpec ret] // ldl_spec expresses RTCDL*, which is the combination of RTCTL* and LDL
...
///////////////////////////////////////////////////////////////////////////////
// ROOT EXPRESSIONS OF RTCTL* operators + epistemic modalities /////////
///////////////////////////////////////////////////////////////////////////////
...
ldl_root_expr			returns[InternalSpec ret]
								: ldl_conc_expr {if(!er()) $ret = $ldl_conc_expr.ret; }
								;

ldl_conc_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_ldl_or_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_COMMA^ s=ldl_ldl_or_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_conc(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

ldl_ldl_or_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_ldl_and_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_LDL_OR^ s=ldl_ldl_and_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_or(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

ldl_ldl_and_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_sere_sat_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_LDL_AND^ s=ldl_sere_sat_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_and(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

ldl_sere_sat_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_sere_imp_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_LDL_SERE_SAT^ s=ldl_sere_imp_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_sere_sat(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

ldl_sere_imp_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_repeat_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_LDL_SERE_IMP^ s=ldl_repeat_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_sere_imp(input, $start, exp_str, $ret, $s.ret); }
								)*
								;

ldl_repeat_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_test_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( 
								lo=TOK_LDL_REPEAT_LB^ r=subrange ro=TOK_RB
									{ if (!er()) exp_str += $lo.text + $r.text + $ro.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_bounded_repeat(input, $start, exp_str, $ret, $r.ret); }
								| lo=TOK_LDL_REPEAT_LB^ ro=TOK_RB
									{ if (!er()) exp_str += $lo.text + $ro.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_repeat(input, $start, exp_str, $ret); }
								)*
								;

ldl_test_expr				returns[InternalSpec ret]
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_simple_root_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( 
								op=TOK_LDL_TEST^
									{ if (!er()) exp_str += $op.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_test(input, $start, exp_str, $ret); }
								)*
								;

///////////////////////////////////////////////////////////////////////////////
// ROOT OF LDL SIMPLE EXPRESSIONS ///////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
ldl_simple_root_expr				returns[InternalSpec ret]
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: f=ldl_relational_expr { if (!er()) exp_str += $f.text; if(!er()) $ret = $f.ret; }
								( op=TOK_AND^ s=ldl_relational_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_and(input, $start, exp_str, $ret, $s.ret); }
								)*
								;
...
@init {boolean append_end = false; String exp_str = ""; }
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								: constant ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $constant.text + " " + $primary_expr_select.text); }
								-> ^(VALUE_T constant NOP ldl_primary_expr_select)
								| ldl_primary_expr_helper1_pointer1
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $primary_expr_helper1_pointer1.text); }
//								| primary_expr_helper1_pointer2
//								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
//								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $primary_expr_helper1_pointer2.text); }
								...
								/* simple paren are the only case where we should start over
								from the "real" begining of the parsed expression... */
								| TOK_LP ldl_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) $ret = $ldl_root_expr.ret; } // primary_expr_select should be null...
								-> ^(BLOCK_T ldl_root_expr NOP ldl_primary_expr_select)
								/* ------------------------------------------------------- */

								// cast has no meaning in a ldl exp. (using the common simple)
								| TOK_BOOL TOK_LP ldl_simple_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_BOOL.text + $TOK_LP.text + $simple_root_expr.text + $TOK_RP.text + $primary_expr_select.text); }
								-> ^(TOK_BOOL ldl_simple_root_expr NOP ldl_primary_expr_select)
								| TOK_WORD1 TOK_LP ldl_simple_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_WORD1.text + $TOK_LP.text + $simple_root_expr.text + $TOK_RP.text + $primary_expr_select.text); }
								-> ^(TOK_WORD1 ldl_simple_root_expr NOP ldl_primary_expr_select)
								// next cannot enforced on a LDL formula. 'AX' should be used.
								| TOK_NEXT TOK_LP ldl_simple_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_NEXT.text + $TOK_LP.text + $simple_root_expr.text + $TOK_RP.text + $primary_expr_select.text); }
								-> ^(TOK_NEXT ldl_simple_root_expr NOP ldl_primary_expr_select)
								// case has no meaning in a LDL exp. (using the common simple)
								| TOK_CASE ldl_case_element_list_expr TOK_ESAC ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_CASE.text + " " + $case_element_list_expr.text + " " + $TOK_ESAC.text + " " + $primary_expr_select.text); }
								-> ^(CASE_LIST_EXPR_T ldl_case_element_list_expr NOP ldl_primary_expr_select)
								// word op has no meaning in a LDL exp. (using the common simple)
								| TOK_WAREAD TOK_LP f=ldl_simple_root_expr TOK_COMMA s=ldl_simple_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_WAREAD.text + $TOK_LP.text + $f.text + $TOK_COMMA.text + $s.text + $TOK_RP.text + $primary_expr_select.text); }
								-> ^(TOK_WAREAD $f $s NOP ldl_primary_expr_select)
								// word op has no meaning in a LDL exp. (using the common simple)
								| TOK_WAWRITE TOK_LP f=ldl_simple_root_expr tc1=TOK_COMMA m=ldl_simple_root_expr tc2=TOK_COMMA s=ldl_simple_root_expr TOK_RP ldl_primary_expr_select
								{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $ldl_primary_expr_helper1.text); }
								//{ if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ref(input, $start, $TOK_WAWRITE.text + $TOK_LP.text + $f.text + $tc1.text + $m.text + $tc2.text + $s.text + $TOK_RP.text + $primary_expr_select.text); }
								-> ^(TOK_WAWRITE $f $m $s NOP ldl_primary_expr_select)
								;

///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
/* simple and common basic subtrees to all LDL simple expressions (operands...) */
///////////////////////////////////////////////////////////////////////////////
///////////////////////////////////////////////////////////////////////////////
ldl_primary_expr_helper1_pointer1	: TOK_ATOM ldl_primary_expr_select
								-> ^(VALUE_T TOK_ATOM NOP ldl_primary_expr_select)
								;
//primary_expr_helper1_pointer2	: TOK_SELF primary_expr_select
//								-> ^(VALUE_T TOK_SELF NOP primary_expr_select)
//								;
ldl_primary_expr_select				: (ldl_primary_expr_select_helper | ldl_primary_expr_ref)*
								;
ldl_primary_expr_ref				: (TOK_DOT! (TOK_ATOM | TOK_NUMBER))
								;
ldl_primary_expr_select_helper		: (TOK_LB ldl_simple_root_expr TOK_RB) => ldl_primary_expr_select_helper_arr_suffix
								| ldl_primary_expr_select_helper_bit_suffix
								;
ldl_primary_expr_select_helper_arr_suffix
								: TOK_LB ldl_simple_root_expr TOK_RB
								-> ^(ARRAY_INDEX_T ldl_simple_root_expr)
								;
ldl_primary_expr_select_helper_bit_suffix
								: TOK_LB f=ldl_simple_root_expr TOK_COLON s=ldl_simple_root_expr TOK_RB
								-> ^(BIT_SELECT_T $f $s)
								;

ldl_case_element_expr				: ldl_simple_root_expr TOK_COLON ldl_simple_root_expr TOK_SEMI
								-> ^(CASE_ELEMENT_EXPR_T ldl_simple_root_expr ldl_simple_root_expr)
								;
ldl_case_element_list_expr			: ldl_case_element_expr (ldl_case_element_expr)*
								;
