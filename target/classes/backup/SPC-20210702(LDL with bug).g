...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								( op=TOK_AND^ s=ldl_ltl_binary_expr
								{ if (!er()) exp_str += " " + $op.text + " " + $s.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_and(input, $start, exp_str, $ret, $s.ret); }
								)*
								;
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								| op=TOK_REPEAT_LB^ r=subrange TOK_RB
...
@after { if(append_end) $ret.setEndToken($stop); if(!er()) $ret.evalBDDChildrenExp(input); }
								...
								// LDL operators
								| lo=TOK_SOMEPATH^ sere=ldl_ltl_unary_expr ro='}' f=ldl_ltl_unary_expr
								{ if (!er()) exp_str = $lo.text + $sere.text + $ro.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_somepath(input, $start, exp_str, $sere.ret, $f.ret); }
								| lo=TOK_ALLPATH^ sere=ldl_ltl_unary_expr ro='}' f=ldl_ltl_unary_expr
								{ if (!er()) exp_str = $lo.text + $sere.text + $ro.text + " " + $f.text; if(!er()) append_end = true; if(!er()) $ret = InitSpec.mk_ldl_allpath(input, $start, exp_str, $sere.ret, $f.ret); }
